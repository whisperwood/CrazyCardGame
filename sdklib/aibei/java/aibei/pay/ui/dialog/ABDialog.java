package com.sdk.aibei.pay.ui.dialog;
...
public class ABDialog extends PayDialog {
	...
	public ABDialog(Context context, Room room) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			if (SdkConstant.PAY_ROOM == 0) {
				...
				for (int i = 0; i < ABConstant.IAPPPAY_LIST.size(); i++) {
					if(Integer.valueOf(money2).equals(ABConstant.IAPPPAY_LIST.get(i).getPrice())){
						limit2 = ABConstant.IAPPPAY_LIST.get(i).getPrice();
						payCode = ABConstant.IAPPPAY_LIST.get(i).getPayCode();
						break;
					}
				}
			} 
				...
				for (int i = 0; i < ABConstant.IAPPPAY_LIST.size(); i++) {
					if (ABConstant.IAPPPAY_LIST.get(i).getPrice() * 10000 < limit2
							|| ABConstant.IAPPPAY_LIST.get(i).getPrice() * 10000 == limit2) {
						limit2 = ABConstant.IAPPPAY_LIST.get(i).getPrice();
						payCode = ABConstant.IAPPPAY_LIST.get(i).getPayCode();
						break;
					} else if (i == ABConstant.IAPPPAY_LIST.size() - 1) {
						limit2 = ABConstant.IAPPPAY_LIST.get(i).getPrice();
	...
	private void layout(final Context context) {
		...
		okBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				ABPayUtil.ABPay(context, String.valueOf(rechargeBean), payCode);
				dismiss();
