...
public class GroupPayActivity extends BaseActivity implements OnClickListener {
	private Button genBackBtn, genRecordBtn, genTelBtn, genAliBtn, genYi2Btn,
			genYi10Btn, genYi30Btn, genZhi10Btn, genZhi20Btn, genZhi30Btn;
	private SharedPreferences sharedPrefrences;
	private Editor editor;
	private TextView zhidou;
	private String simKey;// 标志运营商
	private int[] extrapic_id = { R.id.groupali_1_btn, R.id.groupali_2_btn,
			R.id.groupali_3_btn, R.id.groupali_4_btn };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ThreadPool.startWork(new Runnable() {
			...
			public void run() {
				try {
					String result = HttpUtils.post(HttpURL.PAY_INFO_URL
							+ "info", null, true);
					List<PayPicType> picMap = JsonHelper.fromJson(result,
							new TypeToken<List<PayPicType>>() {
							});
					final int verson = picMap.get(0).getVerson();
					final String picUrl = picMap.get(0).getPicUrl();
					sharedPrefrences = getSharedPreferences("pic_verson",
							MODE_WORLD_READABLE);
					final int versonCode = sharedPrefrences.getInt("newverson",
							0);
						...
						public void run() {
							if (verson != versonCode) {
								...
								for (int i = 0; i < extrapic_id.length; i++) {
									ImageUtil
											.replaceImg(
													HttpURL.PAY_INFO_URL
															+ picUrl,
													(ImageView) findViewById(extrapic_id[i]),
													new ImageCallback() {
														@Override
	...
	private void init() {
		...
		if (Constant.SIM_MOBILE.equals(simType)) {
			...
			PayAdapter adapter = new PayAdapter(GroupPayActivity.this,
					pointList, PaySite.RECHARGE_LIST_FAST);
	...
	public void onClick(View v) {
		...
		case R.id.gen_tel_btn:
			if (ActivityUtils.simExist()
				...
				detailIntent.setClass(GroupPayActivity.this,
						GroupPayDetailActivity.class);
				startActivity(detailIntent);
			} else {
				Toast.makeText(GroupPayActivity.this, "请插入sim卡",
		...
		case R.id.gen_yi_2_btn:
			...
			// SDKFactory.smsPay(Integer.parseInt(GroupConstant.PAY_MONEY_2),
			// SDKConstant.LIST);
			break;
		case R.id.gen_yi_10_btn:
			JDSMSPayUtil.setContext(context);
			PayTipUtils.showTip(10, PaySite.RECHARGE_LIST_FAST);
			// SDKFactory.smsPay(Integer.parseInt(GroupConstant.PAY_MONEY_10),
			// SDKConstant.LIST);
			break;
		case R.id.gen_yi_30_btn:
			JDSMSPayUtil.setContext(context);
			PayTipUtils.showTip(30, PaySite.RECHARGE_LIST_FAST);
			// SDKFactory.smsPay(Integer.parseInt(GroupConstant.PAY_MONEY_30),
			// SDKConstant.LIST);
			break;
