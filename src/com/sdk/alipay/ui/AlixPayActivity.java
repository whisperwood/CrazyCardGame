...
public class AlixPayActivity extends BaseActivity implements IPayView {
	...
	private void initRequestData() {
		...
		mspHelper.setNewestVersion(mPerferences.getBoolean(
				AlipayBroadcastReceiver.NEWEST_SERVICE, false));
		// 判断是否已经安装支付宝安全支付
				...
				try {
					...
					if (retVal == ResultChecker.RESULT_CHECK_SIGN_FAILED) {
						BaseHelper.showDialog(
								...
								onClickListener_Dialog_PositiveButton);
					} else {// 验签成功。验签成功后再判断交易状态码
						if (tradeStatus.equals("9000")) {// 判断交易状态码，只有9000表示交易成功
							// 支付成功
							AlixPayActivity.this.setResult(RESULT_OK,
									new Intent());
							...
							String errMsg = getPayErrorMessage(Integer
									.parseInt("tradeStatus"));
							GameDialog gameDialog = new GameDialog(
									AlixPayActivity.this, false) {
								@Override
								public void okClick() {
									finish();
								};
	...
	private class AliPayTask extends GenericTask {
		...
		protected TaskResult _doInBackground(TaskParams... params) {
			try {
				...
				/** 判断是不是预充值 **/
				if (mBundle != null) {
					String place = mBundle
							.getString(PrerechargeManager.PRERECHARGE_ORDER_PLACE);
					if (PaySite.PREPARERECHARGE.equalsIgnoreCase(place)
				...
				String resultJson = HttpRequest.addPayOrder(
						AliConfig.ALIPAY_URL, paramMap);
				JsonResult result = JsonHelper.fromJson(resultJson,
						JsonResult.class);
				if (JsonResult.SUCCESS.equals(result.getMethodCode())) {
					AliPayOrder payOrder = JsonHelper.fromJson(
							result.getMethodMessage(), AliPayOrder.class);
					...
					sb.append("&");
					sb.append("notify_url=\"").append(payNotifyUrl)
							.append("\"");
