...
public class AliPayActivity extends BaseActivity implements IPayView,
		OnClickListener {
	private EditText editFree;
	private Button aliBackBtn, freeBtn, pay5Btn, pay10Btn, pay20Btn, pay100Btn,
			pay300Btn;
	...
	private int[] extrapic_id = { R.id.extra_pic1, R.id.extra_pic2,
			R.id.extra_pic3, R.id.extra_pic4, R.id.extra_pic5 };
				...
				try {
					...
					if (verson != versonCode) {
						...
						for (int i = 0; i < extrapic_id.length; i++) {
							ImageUtil.replaceImg(HttpURL.PAY_INFO_URL + picUrl,
						...
						for (int i = 0; i < extrapic_id.length; i++) {
							ImageUtil.setImg(HttpURL.PAY_INFO_URL + picUrl,
									(ImageView) findViewById(extrapic_id[i]),
									new ImageCallback() {
										@Override
										public void imageLoaded(Bitmap bitmap,
	...
	protected void onStart() {
		...
		new Thread() {
			@Override
			public void run() {
				final GameUserGoods gameUserGoods = HttpRequest
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							if (point > 10000) {
								...
								zhidou.setText(String.valueOf(gameUser
										.getBean()));
							}
						}
					});
