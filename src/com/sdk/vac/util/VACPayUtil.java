...
public class VACPayUtil {
	...
	// public static void goPay(final String money, final String payFromType,
	// final Room room) {
	// try {
	// ThreadPool.startWork(new Runnable() {
	//
	...
	// /**判断是不是预充值**/
	// if(payFromType != null &&
	// payFromType.equalsIgnoreCase(SDKConstant.PRRECHARGE) && null !=
	// PrerechargeManager.mPayRecordOrder.getPreOrderNo()){
	// paramMap.put(PrerechargeManager.PRERECHARGE_ORDER_PARAMS_PREORDERNO,
	// PrerechargeManager.mPayRecordOrder.getPreOrderNo());
	// paramMap.put(PrerechargeManager.PRERECHARGE_ORDER_PARAMS_PREORDERTYPE,
	// "1");
	// }
	// if(Assistant.ASSID != null && Assistant.BTNCODE != null){
	//
	...
	// }
	// String resultJson = HttpRequest.addPayOrder(VACConfig.VACPAY_URL,
	// paramMap);
	// JsonResult result = JsonHelper.fromJson(resultJson, JsonResult.class);
	// if (JsonResult.SUCCESS.equals(result.getMethodCode())) {
	// final VacPayOrder payOrder =
	// JsonHelper.fromJson(result.getMethodMessage(),
	// VacPayOrder.class);
	// final String gameName =
	// Database.currentActivity.getResources().getString(R.string.app_name);
	// Database.currentActivity.runOnUiThread(new Runnable() {
	//
	// public void run() {
	// MultimodeConfig.setCallbackUrl(payOrder.getPostURL()); // 服务器回调接口
	// MultiModePay.getInstance().vacPay(Database.currentActivity,
	// payOrder.getCompany(), gameName,
	// payOrder.getProductName(), payOrder.getPhone(), money,
	// payOrder.getPayNo(),
	// payOrder.getOriderid(), new VacCallBack() {
	//
	// public void VacResult(String status, String errorMsg) {
	// if (status.equals("00000")) {
	// DialogUtils.toastTip("支付成功");
