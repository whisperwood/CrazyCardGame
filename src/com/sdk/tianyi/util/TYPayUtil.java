			...
			public void run() {
				try {
					...
					String resultJson = HttpRequest.addPayOrder(
							TYConfig.TYPAY_URL, paramMap);
					JsonResult result = JsonHelper.fromJson(resultJson,
							JsonResult.class);
					if (JsonResult.SUCCESS.equals(result.getMethodCode())) {
						final ESurfingOrder payOrder = JsonHelper.fromJson(
								result.getMethodMessage(), ESurfingOrder.class);
						Database.currentActivity.runOnUiThread(new Runnable() {
							@Override
							public void run() {
								Intent intent = new Intent();
								intent.setClass(Database.currentActivity,
										CTEStoreSDKActivity.class);
								Bundle bundle = new Bundle();
								// 示例参数值
								bundle.putString(ApiParameter.APPCHARGEID,
										payOrder.getAppcode());
								bundle.putString(ApiParameter.CHANNELID, "0");
								// int orientation =
								// Database.currentActivity.getResources().getConfiguration().orientation;
								// if(orientation ==
								// Configuration.ORIENTATION_LANDSCAPE){ // 横屏
								// bundle.putBoolean(ApiParameter.SCREENHORIZONTAL,
								// false);//进入横屏XML"
								// }else{
								// bundle.putBoolean(ApiParameter.SCREENHORIZONTAL,
								// true);//进入横屏XML"
								// }
								bundle.putBoolean(
										ApiParameter.SCREENHORIZONTAL, false);// 进入横屏XML"
								// if
								// (paySite.equals(PaySite.RECHARGE_LIST_FAST)
								// ||
								// paySite.equals(PaySite.RECHARGE_LIST)) {
								// bundle.putBoolean(ApiParameter.SCREENHORIZONTAL,
								// false);//进入横屏XML"
								// } else {
								// bundle.putBoolean(ApiParameter.SCREENHORIZONTAL,
								// true);//进入横屏XML"
								// }
								bundle.putString(ApiParameter.CHARGENAME,
										payOrder.getProductName());
								bundle.putInt(ApiParameter.PRICETYPE, 0);
								bundle.putString(ApiParameter.PRICE,
										String.valueOf(point.getMoney()));
								bundle.putString(ApiParameter.REQUESTID,
