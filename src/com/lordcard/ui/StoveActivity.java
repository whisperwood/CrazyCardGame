...
public class StoveActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (bundle != null) {
			...
			if (page == 0) {
				...
				digitalView.setBackgroundDrawable(ImageUtil.getResDrawable(
						R.drawable.gpl_top_left, true));
				helpbutton.setBackgroundDrawable(ImageUtil.getResDrawable(
					...
					gameStove("1", new HttpCallback() {
						...
						public void onSucceed(Object... obj) {
							try {
								...
								Database.GOODS_STOVE_BEAN = JsonHelper
										.fromJson(
												result,
												new TypeToken<List<GoodsExchange>>() {
												});
								handler.sendEmptyMessage(0);
							} catch (Exception e) {
							}
				...
				if (Database.GOODS_STOVE_DIGITAL != null) {
					...
					gameStove("3", new HttpCallback() {
						...
						public void onSucceed(Object... obj) {
							try {
								String result = (String) obj[0];
								Database.GOODS_STOVE_DIGITAL = JsonHelper
										.fromJson(
												result,
												new TypeToken<List<GoodsExchange>>() {
												});
			...
			if (Database.GOODS_STOVE_DIGITAL != null) {
				...
				gameStove("3", new HttpCallback() {
					...
					public void onSucceed(Object... obj) {
						try {
							String result = (String) obj[0];
							Database.GOODS_STOVE_DIGITAL = JsonHelper.fromJson(
									result,
									new TypeToken<List<GoodsExchange>>() {
									});
							handler.sendEmptyMessage(2);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				if (beanSelect != 1) {
					...
					if (null != Database.GOODS_STOVE_BEAN) {
						beanAdapter = new StoveAdapter(StoveActivity.this,
								Database.GOODS_STOVE_BEAN, HC_BEAN);
			...
			case 1:
				...
				// animalAdapter = new StoveAdapter(StoveActivity.this,
				// Database.GOODS_STOVE_ANIMAL, HC_ANIMAL);
			...
			case 2:
				if (degtailSelect != 3) {
					...
					if (null != Database.GOODS_STOVE_DIGITAL) {
						digitalAdapter = new StoveAdapter(StoveActivity.this,
								Database.GOODS_STOVE_DIGITAL, HC_DIGITAL);
			...
			case 4:
				...
				// digitalDetail = new
				// DigtailStoveDialog(Database.currentActivity,
				// R.style.dialog, digiDetail);
				// }
				...
				digitalDetail = new DigtailStoveDialog(
						Database.currentActivity, R.style.dialog, digiDetail);
				digitalDetail.show();
				// if (!Database.StoveDialogShow) {
				// digitalDetail.show();
				// Database.StoveDialogShow = true;
	...
	public void onClick(View v) {
		if (v.getId() == R.id.stove_digital || v.getId() == R.id.stove_bean
				|| v.getId() == R.id.stove_help) {
			beanView.setTextColor(getResources().getColor(
					R.color.gpl_top_btn_text_color));
			digitalView.setTextColor(getResources().getColor(
					R.color.gpl_top_btn_text_color));
			helpbutton.setTextColor(getResources().getColor(
					R.color.gpl_top_btn_text_color));
		}
		switch (v.getId()) {
		case R.id.set_back:
			...
			if (Database.GOODS_STOVE_BEAN != null) {
				...
				gameStove("1", new HttpCallback() {
					...
					public void onSucceed(Object... obj) {
						try {
							String result = (String) obj[0];
							Database.GOODS_STOVE_BEAN = JsonHelper.fromJson(
									result,
									new TypeToken<List<GoodsExchange>>() {
									});
							handler.sendEmptyMessage(0);
	...
	private class StoveAdapter extends BaseAdapter {
		...
		private int type;// listview类型 HC_BEAN：金豆合成 HC_DIGITAL：数码合成
							// HC_ANIMAL：物品合成
		 ...
		 *            :0物品合成 1：数码合成 2：金豆合成
		 */
		public StoveAdapter(Context context, List<GoodsExchange> datalist,
				int type) {
		...
		private void releaseCurrentViews(int type) {
			...
			// for (int i = 1; i < nowListView.getFirstVisiblePosition() - 2;
			// i++) {
			...
			// }
			// }
			// for (int i = nowListView.getLastVisiblePosition() + 2; i <
			// datalist.size(); i++) {
			// View v = nowViewMap.remove(i);
			// if (null != v && !v.isShown()) {
			// v.destroyDrawingCache();
		...
		public View getView(final int position, View convertView,
			...
			if (!viewMap.containsKey(position) || viewMap.get(position) == null) {
				...
				holder.leftiImageView = (ImageView) convertView
						.findViewById(R.id.stove_left_img);
				holder.rightBtn = (Button) convertView
						.findViewById(R.id.stove_right_btn);
				holder.stovenick = (TextView) convertView
						.findViewById(R.id.stove_item_nick);
				holder.stovestuff = (TextView) convertView
						.findViewById(R.id.stove_item_stuff);
			...
			try {
				String path = HttpURL.URL_PIC_ALL
						+ datalist.get(position).getPicPath();
				Log.i("bitmapCache", "HttpURL.URL_PIC_ALL: "
						+ HttpURL.URL_PIC_ALL
						+ "            \n datalist.get(position).getPicPath():"
						+ datalist.get(position).getPicPath());
				switch (type) {
				case HC_BEAN:// 金豆合成
					...
					// (final ImageView view,final String path,final int
					// type,final int
					// position, final Map<Integer, Bitmap> map) {
					break;
				case HC_DIGITAL:// 数码合成
					setImgNoCache(holder.leftiImageView, path, position);
					break;
				...
				String stovecount = "";
				if (datalist.get(position).getCount() != null
						&& datalist.get(position).getCount().intValue() != 1) {
					stovecount = datalist.get(position).getCount().intValue()
							+ "";
				}
				holder.stovenick.setText(stovecount
						+ datalist.get(position).getTypeName());
				List<GoodsPart> goodsParts = datalist.get(position)
						.getGoodsParts();
				String stoveAllstuff = "";
				for (int i = 0; i < goodsParts.size(); i++) {
					if (goodsParts.get(i).getUnit() != null) {
						if (!TextUtils.isEmpty(goodsParts.get(i).getFromName()
								.trim())
								&& !"等级".equals(goodsParts.get(i).getFromName()
										.trim())) {
							stoveAllstuff += "+"
									+ goodsParts.get(i).getFromCount()
											.toString()
									+ goodsParts.get(i).getUnit()
									+ goodsParts.get(i).getFromName();
						}
					} else {
						if (!TextUtils.isEmpty(goodsParts.get(i).getFromName()
								.trim())
				...
				holder.stovestuff.setText(stoveAllstuff,
							...
							ThreadPool.startWork(new Runnable() {
								...
								public void run() {
									...
									String result = HttpUtils.post(
											HttpURL.STOVE_DIGIDETAIL, paramMap,
											true);
									digiDetail = JsonHelper.fromJson(result,
											new TypeToken<GoodsTypeDetail>() {
											});
					...
					public void onClick(View v) {
						stvDialog = new StoveDialog(Database.currentActivity,
								R.style.dialog, datalist.get(position)
										...
										.getType(), datalist.get(position)
										.getTypeId(), datalist.get(position)
										.getCount(), datalist.get(position)
										.getGoodsParts());
							...
							public void run() {
								...
								if (imageurl != null && imageurl.size() > 0) {
									for (int j = 0; j < imageurl.size(); j++) {
										if (stuff.indexOf(imageurl.get(j)
												.getName()) != -1) {
											int star = stuff.indexOf(imageurl
													.get(j).getName());
											int end = stuff.indexOf(imageurl
													.get(j).getName())
													+ imageurl.get(j).getName()
															.length();
											Spannable spn = (Spannable) tv
													.getText();
											spn.setSpan(
	...
	private void gethelp() {
		if (Database.STOVE_GUIDE_LIST != null
				&& Database.STOVE_GUIDE_LIST.size() > 0) {
				...
				public void run() {
					try {
						...
						String result = HttpUtils.post(HttpURL.GAME_GUIDE_URL,
								paramMap, true);
						Database.STOVE_GUIDE_LIST = JsonHelper.fromJson(result,
	...
	private class HelpAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			holder.guideItem.setText(datalist.get(position).getTitle()
					.replace("?", "")
					+ "  ");
			detailtextList = JsonHelper.fromJson(helpDatalist.get(position)
	...
	protected void onDestroy() {
		...
		try {
			if (null != beanViewMap) {
				Iterator<Map.Entry<Integer, View>> it = beanViewMap.entrySet()
						.iterator();
				while (it.hasNext()) {
					try {
			...
			if (null != digitalViewMap) {
				Iterator<Map.Entry<Integer, View>> it = digitalViewMap
						.entrySet().iterator();
				while (it.hasNext()) {
					try {
						View v = it.next().getValue();
	...
	private void setImgNoCache(final ImageView view, final String path,
			final int position) {
