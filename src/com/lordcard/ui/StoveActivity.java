...
public class StoveActivity extends BaseActivity implements OnClickListener {
					...
					gameStove("1", new HttpCallback() {
						...
						public void onSucceed(Object... obj) {
							try {
								...
								Database.GOODS_STOVE_BEAN = JsonHelper.fromJson(result,
				...
				if (Database.GOODS_STOVE_DIGITAL != null) {
					...
					gameStove("3", new HttpCallback() {
						...
						public void onSucceed(Object... obj) {
							try {
								String result = (String) obj[0];
								Database.GOODS_STOVE_DIGITAL = JsonHelper.fromJson(result,
										new TypeToken<List<GoodsExchange>>() {
										});
								handler.sendEmptyMessage(2);
							} catch (Exception e) {
				...
				case 2:
					if (degtailSelect != 3) {
						...
						if (null != Database.GOODS_STOVE_DIGITAL) {
							digitalAdapter = new StoveAdapter(StoveActivity.this, Database.GOODS_STOVE_DIGITAL,
									HC_DIGITAL);
			...
			try {
				...
				for (int i = 0; i < goodsParts.size(); i++) {
					if (goodsParts.get(i).getUnit() != null) {
						if (!TextUtils.isEmpty(goodsParts.get(i).getFromName().trim())
								&& !"等级".equals(goodsParts.get(i).getFromName().trim())) {
							stoveAllstuff += "+" + goodsParts.get(i).getFromCount().toString()
									+ goodsParts.get(i).getUnit() + goodsParts.get(i).getFromName();
						}
					} else {
						if (!TextUtils.isEmpty(goodsParts.get(i).getFromName().trim())
								&& !"等级".equals(goodsParts.get(i).getFromName().trim())) {
				...
				holder.rightBtn.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						stvDialog = new StoveDialog(Database.currentActivity, R.style.dialog, datalist.get(position)
								.getTypeName(), datalist.get(position).getType(), datalist.get(position).getTypeId(),
								datalist.get(position).getCount(), datalist.get(position).getGoodsParts());
								...
								if (imageurl != null && imageurl.size() > 0) {
									for (int j = 0; j < imageurl.size(); j++) {
										if (stuff.indexOf(imageurl.get(j).getName()) != -1) {
											...
											int end = stuff.indexOf(imageurl.get(j).getName())
													+ imageurl.get(j).getName().length();
											Spannable spn = (Spannable) tv.getText();
											spn.setSpan(new ForegroundColorSpan(Color.YELLOW), star, end,
													Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
											spn.setSpan(new StyleSpan(android.graphics.Typeface.ITALIC), star, end,
													Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
										}
									}
								}
	...
	private class HelpAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			detailtextList = JsonHelper.fromJson(helpDatalist.get(position).getDescription(),
