...
public class DoudizhuMainGameActivity extends BaseActivity implements IGameView, OnTouchListener, HasTiShiListenner,
			...
			if (id == R.id.playimageView1 || id == R.id.zhezhao1) { // 位置1玩家图像
				...
				switch (id) {
						...
						if (0 != masterOrder) {
							if (isJiPaiQiEnable || isFirstTimeUseJiPaiQiOneDay()) {
								...
								if (null == jiPaiQiChargeDialog)
									jiPaiQiChargeDialog = new JiPaiQiChargeDialog(DoudizhuMainGameActivity.this,
											jiPaiQiTipsMsg);
					...
					case R.id.dzed_borrow_beans_btn:// 借点豆
						...
						if ((!isShowPrerechargeDialog) && (cacheUser.getBean() >= 0)
								&& (currentWager > cacheUser.getBean())) {
							PrerechargeManager.createPrerechargeDialog(PrerechargeDialogType.Dialog_ingame,
	...
	/**
	 * 加倍
	 * @param multiple 加倍的倍数( 2:加2倍 4:加4倍)
	 */
	...
	private void clickBaoxiang() {
		...
		if (6 == Database.JOIN_ROOM.getHomeType()) {
			...
			if (quang < 5 && Math.abs(System.currentTimeMillis() - Constant.CLICK_TIME) >= Constant.SPACING_TIME) {
				...
				try {
					SharedPreferences sharedData = getApplication().getSharedPreferences(Constant.GAME_ACTIVITE,
				...
				try {
					...
					editor.putInt(Constant.QUANG_KEY, 0);
					editor.commit();
					SharedPreferences sharedPrefer = getApplication().getSharedPreferences(Constant.GAME_GETFEE,
							Context.MODE_PRIVATE);
	...
	public void showPrerechargeLl() {
		...
		if ((cacheUser.getBean() >= 0) && (currentWager > cacheUser.getBean())) {
			beansInsufficientTv.setText("本局可赢" + PatternUtils.changeZhidou(currentWager) + "，您的金豆不足只能赢"
			...
			// 地主产生后才显示
			if ((beansInsufficientRl.getVisibility() != View.VISIBLE) && (rechargeLl.getVisibility() != View.VISIBLE)
					&& 0 != masterOrder) {
	...
	public void showPrerechargeDialog() {
		/**
		 * 1、如果当局还没有显示过预充值界面 2、当前剩余牌数小于等于@PrerechargeManager.showPrerechargelastCardCount
		 * 3、当前金豆大于等于0 4、当前赌注大于预充值条件
		 **/
		GameUser cacheUser = (GameUser) GameCache.getObj(CacheKey.GAME_USER);
		int allDouble = getCallDoubleNum();
		long currentWager = (Integer.valueOf(beishuNumber) * Database.JOIN_ROOM_BASEPOINT) * allDouble;
		if ((!isShowPrerechargeDialog) && (nowcard.size() <= PrerechargeManager.showPrerechargelastCardCount)
				&& (cacheUser.getBean() >= 0)
				&& (currentWager > (PrerechargeManager.showPrerechargePercent * cacheUser.getBean()))) {
			PrerechargeManager.createPrerechargeDialog(PrerechargeDialogType.Dialog_ingame,
					DoudizhuMainGameActivity.this, null, null, Integer.valueOf(beishuNumber) * allDouble).show();
			isShowPrerechargeDialog = true;
		}
	}
	...
	private void initView() {
		...
		Map<String, String> roomRuleMap = JsonHelper.fromJson(Database.JOIN_ROOM.getRule(),
				new TypeToken<Map<String, String>>() {
				});
		if (6 == Database.JOIN_ROOM.getHomeType()) {
			if (roomRuleMap.containsKey("path")) {
				baoxiang.setBackgroundDrawable(new BitmapDrawable(null == ImageUtil.getGirlBitmap(HttpURL.URL_PIC_ALL
						+ roomRuleMap.get("path"), true, false) ? BitmapFactory.decodeResource(ctx.getResources(),
						R.drawable.baoxiang) : ImageUtil.getGirlBitmap(HttpURL.URL_PIC_ALL + roomRuleMap.get("path"),
						true, false)));
			} else {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
					case 0: // 发牌更新界面
						...
						// // 自己
						playTextView1
								.setText(TextUtils.isEmpty(mGameUser.getNickname()) ? "" : mGameUser.getNickname()); // 自己
						...
						try {
							for (Entry<String, String> entry : dpMap.entrySet()) {
								final int count = TextUtils.isEmpty(entry.getKey()) ? 0 : Integer.parseInt(entry
										.getKey());
											...
											if (null != bitmap) {
												for (int i = 0; i < count; i++) {
													...
													RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
															mst.adjustXIgnoreDensity(DP_WIDTH), mst
																	.adjustYIgnoreDensity(DP_HEIGHT));
													if (i < 3) {
														params.leftMargin = mst.adjustXIgnoreDensity(DP_WIDTH * i);
													} else {
														params.topMargin = mst.adjustYIgnoreDensity(DP_HEIGHT / 2
																+ DP_PANDING);
														params.leftMargin = mst.adjustXIgnoreDensity(DP_WIDTH / 2
																+ DP_WIDTH * (i - 3));
													}
					...
					case 22:// 淘汰或比赛结束
						if (!TextUtils.isEmpty(gameOverDetail)) {
							...
							Map<String, String> map = JsonHelper.fromJson(gameOverDetail,
									new TypeToken<Map<String, String>>() {
									});
			...
			private void getTiLaMsg(Message msg) {
				...
				if (null != tila.getOrder() && 0 != tila.getRatio()) {
					...
					Log.i("Ordersss", "tila.getOrder():" + tila.getOrder() + "      mySelfOrder:" + mySelfOrder
	...
	/**
	 * 自己打牌
	 * @param comeOnFling 是否为onFling调用
	 */
	...
	public void setDizhuIcon(int msterOrder) {
		if (msterOrder == mySelfOrder) { // 如果自己是地主的话
			...
			if (null != icon2) {
				ActivityUtils.setHead(ctx, icon2, Database.userMap.get(msterOrder).getGender(), true, Database.userMap
						.get(msterOrder).getIqImg(), true);// 地主
			}
			int po3 = getNextOrder(msterOrder);
			if (po3 == mySelfOrder) {
				ActivityUtils.setHead(ctx, play1Icon, Database.userMap.get(mySelfOrder).getGender(), false,
						Database.userMap.get(mySelfOrder).getIqImg(), true);// 农民
			} else {
				ImageView icon3 = (ImageView) findViewById(1300 + po3);
				if (null != icon3) {
					ActivityUtils.setHead(ctx, icon3, Database.userMap.get(po3).getGender(), false, Database.userMap
							.get(po3).getIqImg(), true);// 农民
				}
			}
			...
			if (po4 == mySelfOrder) {
				...
				if (null != icon4) {
					ActivityUtils.setHead(ctx, icon4, Database.userMap.get(po4).getGender(), false, Database.userMap
							.get(po4).getIqImg(), true);// 农民
				}
			}
		...
		if (id.equals(mySelfId)) {
			...
			for (int i = 0; i < 3; i++) {
				...
				ca.getPokeImage().setImageDrawable(
						ImageUtil.getResDrawable(poker[Integer.parseInt(dizhuCard[i])].getBitpamResID(), true));
				RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(mst.adjustXIgnoreDensity(50),
						mst.adjustYIgnoreDensity(68));
				params.leftMargin = mst.adjustXIgnoreDensity(13 * i);
				dizhuPaiLayout.addView(ca, params);
		...
		if (str.equals("")) {
			...
			for (int i = 0; i < otherPlayCards.length; i++) {
				...
				c.getPokeImage().setImageDrawable(
						ImageUtil.getResDrawable(poker[Integer.parseInt(otherPlayCards[i])].getBitpamResID(), true));
			...
			playDiZhuCardAudio(type, value, TextUtils.isEmpty(Database.userMap.get(playOrder).getGender()) ? "0"
					: Database.userMap.get(playOrder).getGender()); // 出牌的声音
			// 如果是炸弹显示特效
		...
		if (0 != masterOrder) {
			...
			if (Math.abs(System.currentTimeMillis() - Constant.CLICK_TIME) >= Constant.SPACING_TIME) {// 防止重复刷新
				...
				try {
					...
					PhotoDialog photoDialog = new PhotoDialog(view, isMaster, DoudizhuMainGameActivity.this,
							showAccount, gu);
	...
	private class GameCallBack implements ICallback {
		...
		public synchronized void messageHandler(CmdDetail cmdDtl) {
			...
			if (CmdUtils.CMD_USER.equals(cmd)) { // 用户信息
				Database.userMap = JsonHelper.fromJson(detail, new TypeToken<Map<Integer, GameUser>>() {
				}); // 用户的基本信息
				if (Database.userMap != null) {
					GameUser users = null;
					GameUser cacheUser = (GameUser) GameCache.getObj(CacheKey.GAME_USER);
			...
			if (CmdUtils.CMD_RANK.equals(cmd)) { // 排名（普通赛制）
				...
				final List<GameScoreTradeRank> gstList = JsonHelper.fromJson(detail,
						new TypeToken<ArrayList<GameScoreTradeRank>>() {
						});
			...
			if (CmdUtils.CMD_PPC.equals(cmd)) {
				...
				if (null != status && status.equalsIgnoreCase(Constant.SUCCESS)) {
					...
					DialogUtils.toastTip(Database.currentActivity
		...
		try {
			...
			if (relink.getTarget() == ReLink.TARGET_OTHER) {
				...
				if (relinkOrder == getNextOrder(mySelfOrder)) { // 我的下家连线
					...
					ActivityUtils.setHead(ctx, play2Icon, Database.userMap.get(p2o).getGender(), (p2o == masterOrder),
							Database.userMap.get(p2o).getIqImg(), false);// 设置头像
				} else if (relinkOrder == getPerOrder(mySelfOrder)) {// 我的上家连线
					int p3o = getPerOrder(mySelfOrder);
					GameUser mGameUser3 = Database.userMap.get(p3o);
					ActivityUtils.setHead(ctx, play3Icon, mGameUser3.getGender(), (p3o == masterOrder),
			...
			ActivityUtils.setHead(ctx, play1Icon, Database.userMap.get(mySelfOrder).getGender().trim(),
					(mySelfOrder == relink.getMasterOrder()), Database.userMap.get(mySelfOrder).getIqImg(), false);// 设置头像
							...
							public void imageLoaded(Bitmap bitmap, ImageView view) {
								if (null != bitmap) {
									for (int i = 0; i < count; i++) {
										ImageView img = new ImageView(ctx);
										img.setBackgroundDrawable(new BitmapDrawable(bitmap));
										RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(mst
												.adjustXIgnoreDensity(DP_WIDTH), mst.adjustYIgnoreDensity(DP_HEIGHT));
										if (i < 3) {
											params.leftMargin = mst.adjustXIgnoreDensity(DP_WIDTH * i);
										} else {
											params.topMargin = mst.adjustYIgnoreDensity(DP_HEIGHT / 2 + DP_PANDING);
											params.leftMargin = mst.adjustXIgnoreDensity(DP_WIDTH / 2 + DP_WIDTH
													* (i - 3));
										}
										img.setLayoutParams(params);
										dpRl1.addView(img);
			...
			ActivityUtils.setHead(ctx, play2Icon, Database.userMap.get(p2o).getGender(),
					(p2o == relink.getMasterOrder()), Database.userMap.get(p2o).getIqImg(), false);// 设置头像
			play2Order.setText(String.valueOf(p2o));
			...
			ActivityUtils.setHead(ctx, play3Icon, Database.userMap.get(p3o).getGender(),
					(p3o == relink.getMasterOrder()), Database.userMap.get(p3o).getIqImg(), false);// 设置头像
			play3Order.setText(String.valueOf(p3o));
			...
			if (relStatus == Constant.STATUS_PLAYING) { // 正在打牌时重连
				...
				try {
					...
					for (int i = 0; i < dzCard.length; i++) {
						Poker ca = new Poker(this);
						ca.getPokeImage().setImageDrawable(
								ImageUtil.getResDrawable(poker[Integer.parseInt(dzCard[i])].getBitpamResID(), true));
						RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
					...
					if (pcardList.size() == 0) { // 自己不出
						...
						for (int j = 0; j < passPokerList.size(); j++) {
							...
							image.getPokeImage().setImageDrawable(
									ImageUtil.getResDrawable(passPokerList.get(j).getBitpamResID(), true));
	...
	public boolean onSingleTapUp(MotionEvent e) {
		...
		if (e.getRawY() > play3Icon.getHeight()) {
			...
			if (PreferenceHelper.getMyPreference().getSetting().getBoolean("shoushi", true)) {
				...
				/** 加倍选择 */
				if (tilaLayout.getVisibility() == View.VISIBLE && jiaofenLayout.getVisibility() != View.VISIBLE
						&& playBtnLayout.getVisibility() != View.VISIBLE) {
					/** 间距 */
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						Drawable draw1 = ImageUtil.getcutBitmap(HttpURL.URL_PIC_ALL + girls.get(postion).get("path"),
								false);
						if (null != draw1) {
							DoudizhuMainGameActivity.this.viewFlipper.getChildAt(curpostion).setBackgroundDrawable(
									draw1);
						}
