...
public class DoudizhuMainGameActivity extends BaseActivity implements
	...
	private AutoTask selfTask, leftTask, rightTask, pubTask, adTask, gameTask,
			task2, baoXiangTask, headTask, prerechargeTask;
			...
			if (id == R.id.playimageView1 || id == R.id.zhezhao1) { // 位置1玩家图像
				...
				if (!TextUtils.isEmpty(p3o)) {
					photoClick(play3Icon, Integer.parseInt(p3o),
							(PLAY3ICON_ID - 1300) == masterOrder);
				}
			} else {
				switch (id) {
				case R.id.mess_btn_view:
					if (Math.abs(System.currentTimeMillis()
							- Constant.CLICK_TIME) >= Constant.SPACING_TIME) {// 防止重复刷新
				...
				case R.id.chupai_button:// 出牌
					...
					if (null != myTime) {
						int time = Integer
								.parseInt(myTime.getText().toString());
						if (time > 0 && time != 20) {
							playCard(false);
						}
					}
				...
				case R.id.gp_top_btn:// 排名(快速赛制)
					if (1 == Database.JOIN_ROOM.getRoomType()) {
						if (Math.abs(System.currentTimeMillis()
								- Constant.CLICK_TIME) >= Constant.SPACING_TIME) {
							Constant.CLICK_TIME = System.currentTimeMillis();
					...
					if (0 != masterOrder) {
						if (isJiPaiQiEnable || isFirstTimeUseJiPaiQiOneDay()) {
							if (leftJiPaiQiLayout.getVisibility() == View.VISIBLE)
								...
								if (null != beansInsufficientRl
										&& beansInsufficientRl.getVisibility() == View.VISIBLE) {
							...
							if (null == jiPaiQiChargeDialog)
								jiPaiQiChargeDialog = new JiPaiQiChargeDialog(
										DoudizhuMainGameActivity.this,
										jiPaiQiTipsMsg);
							Log.i("JI_PAI_QI_FREE_COUNT", "jiPaiQiTipsMsg:"
									+ jiPaiQiTipsMsg);
				...
				case R.id.dzed_borrow_beans_btn:// 借点豆
					...
					long currentWager = (Integer.valueOf(beishuNumber) * Database.JOIN_ROOM_BASEPOINT)
							* allDouble;
					if ((!isShowPrerechargeDialog)
							&& (cacheUser.getBean() >= 0)
							&& (currentWager > cacheUser.getBean())) {
						PrerechargeManager.createPrerechargeDialog(
								PrerechargeDialogType.Dialog_ingame,
								DoudizhuMainGameActivity.this, null, null,
								Integer.valueOf(beishuNumber) * allDouble)
								.show();
	...
	private void rotationDizhuRl() {
		...
		animation.setAnimationListener(new Animation.AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				mInfoRl.startAnimation(AnimationUtils.loadAnimation(ctx,
						R.anim.front_scale));
			}
		});
	...
	private void clickBaoxiang() {
		...
		if (6 == Database.JOIN_ROOM.getHomeType()) {
			...
			if (quang < 5
				...
				try {
					SharedPreferences sharedData = getApplication()
							.getSharedPreferences(Constant.GAME_ACTIVITE,
				...
				try {
					...
					editor.putInt(Constant.QUANG_KEY, 0);
					editor.commit();
					SharedPreferences sharedPrefer = getApplication()
							.getSharedPreferences(Constant.GAME_GETFEE,
									Context.MODE_PRIVATE);
		...
		popThread = new Thread() {
			...
			public void run() {
				if (usetool == null) {
					...
					String result = HttpUtils.post(HttpURL.MAIN_DAOJU_URL,
							paramMap, true);
					try {
	...
	protected void initPopuptWindow() {
		...
		popupWindow_view = getLayoutInflater().inflate(R.layout.pop, null,
				false);
		mst.unRegisterView(popupWindow_view
				.findViewById(R.id.login_sliding_content));
		mst.adjustView(popupWindow_view
				.findViewById(R.id.login_sliding_content));
	...
	public boolean isFirstTimeUseJiPaiQiOneDay() {
		Log.i("JI_PAI_QI_FREE_COUNT", "Database.JI_PAI_QI_FREE_COUNT:"
		...
		if (null != timeString) {
			...
			if (date.getDate() != calendar.get(Calendar.DATE)
					|| (date.getDate() == calendar.get(Calendar.DATE) && (0 != Database.JI_PAI_QI_FREE_COUNT))) {
				if (Database.JI_PAI_QI_FREE_COUNT >= 1) {
					Database.JI_PAI_QI_FREE_COUNT--;
					ActivityUtils.addSharedValue(
	...
	/**
	 * 设置记牌器是否可用 1.地主为产生(0==masterOrder)，不能使用 2.当天使用次数已经达到当天可使用次数后，不能使用
	 */
	private void setJipaiqiAvailableOrNotAvailable() {
		Calendar calendar = Calendar.getInstance(Locale.CHINA);
		String timeString2 = ActivityUtils
				.getSharedValue(RecordPorkerView.JIPAIQI_USE_TIME);
		if (0 == masterOrder) {
			btn_jipaiqi
	...
	public void showPrerechargeLl() {
		...
		if ((cacheUser.getBean() >= 0) && (currentWager > cacheUser.getBean())) {
			beansInsufficientTv.setText("本局可赢"
					+ PatternUtils.changeZhidou(currentWager) + "，您的金豆不足只能赢"
					+ PatternUtils.changeZhidou(cacheUser.getBean()));
			Log.i("freshUserInfo", "您的金豆不足只能赢" + cacheUser.getBean());
			// int[] price =
			// PrerechargeManager.calculatePrerechargePrice(cacheUser.getBean(),
			// currentWager, ActivityUtils.getSimType());
			int[] price = PrerechargeManager.calculatePrerechargePrice(
					cacheUser.getBean(), currentWager);
			...
			// 地主产生后才显示
			if ((beansInsufficientRl.getVisibility() != View.VISIBLE)
					&& (rechargeLl.getVisibility() != View.VISIBLE)
					&& 0 != masterOrder) {
	...
	public void showPrerechargeDialog() {
		...
		long currentWager = (Integer.valueOf(beishuNumber) * Database.JOIN_ROOM_BASEPOINT)
				* allDouble;
		if ((!isShowPrerechargeDialog)
				&& (nowcard.size() <= PrerechargeManager.showPrerechargelastCardCount)
				&& (cacheUser.getBean() >= 0)
				&& (currentWager > (PrerechargeManager.showPrerechargePercent * cacheUser
						.getBean()))) {
			PrerechargeManager.createPrerechargeDialog(
					PrerechargeDialogType.Dialog_ingame,
					DoudizhuMainGameActivity.this, null, null,
					Integer.valueOf(beishuNumber) * allDouble).show();
			isShowPrerechargeDialog = true;
		}
	}
	...
	public void showBaoXiang(boolean isAdd) {
		if (6 == Database.JOIN_ROOM.getHomeType()) {
			...
			if (0 != quang && quang == allQuan) {
				...
				AnimUtils.playAnim(baoXiangStar,
						ImageUtil.getResAnimaSoft("baoxiang_start_"), 0);
		...
		new Thread() {
			...
			public void run() {
				try {
					...
					String count = HttpUtils.post(HttpURL.COUPON_GIVE_URL,
							paramMap);
					if (count.equals("0")) {
						if (6 == Database.JOIN_ROOM.getHomeType()) {
							DialogUtils.mesToastTip("亲，您目前未能获得钻石，继续加油吧！");
	...
	public void sendTextMessage(String talk, int clickType) {
		if (talk.equals("")) {
			Toast.makeText(DoudizhuMainGameActivity.this, "发送消息不能为空",
	 ...
	 *            是否是本地叫
	 */
	private void callPoints(int tempRatio, ImageView info, int order,
			boolean isLocal) {
	...
	private void initView() {
		...
		Map<String, String> roomRuleMap = JsonHelper.fromJson(
				Database.JOIN_ROOM.getRule(),
				new TypeToken<Map<String, String>>() {
				});
		if (6 == Database.JOIN_ROOM.getHomeType()) {
			if (roomRuleMap.containsKey("path")) {
				baoxiang.setBackgroundDrawable(new BitmapDrawable(
						null == ImageUtil.getGirlBitmap(HttpURL.URL_PIC_ALL
								+ roomRuleMap.get("path"), true, false) ? BitmapFactory
								.decodeResource(ctx.getResources(),
										R.drawable.baoxiang) : ImageUtil
								.getGirlBitmap(HttpURL.URL_PIC_ALL
										+ roomRuleMap.get("path"), true, false)));
			} else {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 0: // 发牌更新界面
					...
					// // 自己
					playTextView1.setText(TextUtils.isEmpty(mGameUser
							.getNickname()) ? "" : mGameUser.getNickname()); // 自己
					if (!TextUtils.isEmpty(mGameUser.getNickname())) {// 保存自己的名字供单机使用
						GameCache.putStr(Constant.GAME_NAME_CACHE,
								mGameUser.getNickname());
					}
					// ActivityUtils.setDefaultHead(ctx, play1Icon,
					// mGameUser.getGender(),
					// false);
					...
					try {
						for (Entry<String, String> entry : dpMap.entrySet()) {
							...
							if (!TextUtils.isEmpty(path)) {
								...
								ImageUtil.setImg(HttpURL.URL_PIC_ALL + path,
												...
												if (null != bitmap) {
													for (int i = 0; i < count; i++) {
														...
														if (i < 3) {
															...
															params.topMargin = mst
																	.adjustYIgnoreDensity(DP_HEIGHT
																			/ 2
																			+ DP_PANDING);
															params.leftMargin = mst
																	.adjustXIgnoreDensity(DP_WIDTH
					...
					// ActivityUtils.setDefaultHead(ctx, play2Icon,
					// mGameUser2.getGender(),
					// false);
					...
					// ActivityUtils.setDefaultHead(ctx, play3Icon,
					// mGameUser3.getGender(),
					// false);
				...
				case 10: // 接收显示地主的牌（带踢拉功能）
					...
					LastCards lastCard = (LastCards) msg.getData().get(
							"lastCard");
					// 更新地主牌数量
				...
				case 18:// 收到聊天定时器消息
					CmdDetail mess = (CmdDetail) msg.getData().get(
							CmdUtils.CMD_CHAT);
					/*
					 * try { mess.setDetail(new
					 * String(Base64Util.decode(mess.getDetail()),"UTF-8")); }
				...
				case 22:// 淘汰或比赛结束
					if (!TextUtils.isEmpty(gameOverDetail)) {
						...
						if ("-1".equals(prize)) {// -1表示为没人任何奖品
							...
							prizeGoods = JsonHelper.fromJson(prize,
									new TypeToken<List<PrizeGoods>>() {
									});
						}
				...
				case 302:// 隐藏宝箱
					if (null != Database.JOIN_ROOM
							&& 6 == Database.JOIN_ROOM.getHomeType()) {
						if (quang < allQuan) {
							AnimUtils.startAnimationsOut(baoXiangLayout, 300,
									50);
						}
					} else {
						if (quang < 5) {
				...
				case 403:// 其他玩家断线通知
					...
					if (breakOrder == getNextOrder(mySelfOrder)) { // 我的下家断线
						play2Icon.setImageDrawable(ImageUtil.getResDrawable(
								R.drawable.robot, true));
						play2IsTuoGuan = true;
					} else if (breakOrder == getPerOrder(mySelfOrder)) {// 我的上家断线
						play3Icon.setImageDrawable(ImageUtil.getResDrawable(
								R.drawable.robot, true));
						play3IsTuoGuan = true;
					}
					break;
				case 404:// 显示比赛场积分\排名等信息
					...
					gpScore.setText("积分"
							+ PatternUtils.changeZhidou(gu.getCred()));
				...
				case 666:// 收到金豆超过上限的命令
					...
					/*
					 * try { tipsMess = new
					 * String(Base64Util.decode(tipsMess),"UTF-8"); } catch
			...
			private void hasDiZhu(Message msg) {
				...
				genxinMycard(grabMaster.getId(), grabMaster.getLastcards(),
			...
			private void getTiLaMsg(Message msg) {
				...
				if (null != Database.userMap
						&& Database.userMap.containsKey(tila.getOrder())) {
				...
				if (null != tila.getOrder() && 0 != tila.getRatio()) {
					...
					if (1 != tila.getRatio()) {// 加倍
						if (2 == tila.getRatio()) {
							info.setBackgroundDrawable(ImageUtil
									.getResDrawable(R.drawable.double_2, true));
							// 显示已加倍的状态
							if (mySelfOrder == tila.getOrder()) {
								jiabei1Iv.setVisibility(View.VISIBLE);
								jiabei1Iv
										.setImageResource(R.drawable.jiabei_x_2);
							}
							...
							info.setBackgroundDrawable(ImageUtil
									.getResDrawable(R.drawable.double_4, true));
							// 显示已加倍的状态
							if (mySelfOrder == tila.getOrder()) {
								jiabei1Iv.setVisibility(View.VISIBLE);
								jiabei1Iv
										.setImageResource(R.drawable.jiabei_x_4);
							}
							if (JIABEI2_ID == (1600 + tila.getOrder())) {
								jiabei2Iv.setVisibility(View.VISIBLE);
								jiabei2Iv
					...
					Log.i("Ordersss", "tila.getOrder():" + tila.getOrder()
							+ "      mySelfOrder:" + mySelfOrder
	 ...
	 * @param prizeGoods2
	 */
	private void showGameOverDialog(final String rank,
			final List<PrizeGoods> prizeGoods2) {
		god = new GameOverDialog(DoudizhuMainGameActivity.this, R.style.dialog,
		...
		if (null != god && !god.isShowing()) {
			...
			android.view.WindowManager.LayoutParams lay = god.getWindow()
					.getAttributes();
			setParams(lay);
			god.show();
			gameOverDetail = "";
	...
	private void playCard(boolean comeOnFling) {
		...
		if (!firstChupai) {
			...
			if (DoudizhuRule.compterpai(typeplay1, typeMe,
				...
				playDiZhuCardAudio(typeMe, valueMe,
						Database.userMap.get(mySelfOrder).getGender()); // 出牌的声音
				// 如果是炸弹显示特效
	...
	// holder.iv.setBackgroundDrawable(ImageUtil.getDrawableResId(gifInt[position],
	// true, true));
	...
	public void setEndDonghua(final LinkedList<Play> users) {
		...
		for (Play end : userLink) {
			// 如果是自己的顺序
			String[] last = end.getCards()
					.substring(1, end.getCards().length() - 1).split(",");
		...
		if (null != beansInsufficientRl
				&& View.VISIBLE == beansInsufficientRl.getVisibility()) {
			beansInsufficientRl.setVisibility(View.GONE);
		}
		...
		AutoTask goOutTask = new AutoTask() {
			...
			public void run() {
				ctx.runOnUiThread(new Runnable() {
					...
					public void run() {
						dismissDialog();
						GameEndDialog mGameEndDialog = new GameEndDialog(ctx,
								users, mySelfOrder, handler);
						mGameEndDialog.setContentView(R.layout.doudizhu_end);
	...
	public void cardAddview(List<Poker> passCardList, boolean sigleTime) {
		...
		for (int i = 0; i < passCardList.size(); i++) {
			...
			for (int j = 0; j < nowcard.size(); j++) {
				if (nowcard.size() > 1) {
					card_jiange = (800 - 90) / (nowcard.size() - 1);
					if (card_jiange > 50) {
						card_jiange = 50;
					}
				} else {
	...
	public void setDizhuIcon(int msterOrder) {
		if (msterOrder == mySelfOrder) { // 如果自己是地主的话
			GameUser mGameUser = Database.userMap.get(mySelfOrder);
			ActivityUtils.setHead(ctx, play1Icon, mGameUser.getGender(), true,
					mGameUser.getIqImg(), true);// 地主
			...
			if (null != icon) {
				ActivityUtils.setHead(ctx, icon, mGameUser1.getGender(), false,
						mGameUser1.getIqImg(), true);// 农民
			}
			int po2 = getNextOrder(po1);
			GameUser mGameUser2 = Database.userMap.get(po2);
			ImageView icon1 = (ImageView) findViewById(1300 + po2);
			if (null != icon1) {
				ActivityUtils.setHead(ctx, icon1, mGameUser2.getGender(),
						false, mGameUser2.getIqImg(), true);// 农民
			}
			zhezhao1.setVisibility(View.GONE);
		} else {
			ImageView icon2 = (ImageView) findViewById(1300 + msterOrder);
			if (null != icon2) {
				ActivityUtils.setHead(ctx, icon2,
						Database.userMap.get(msterOrder).getGender(), true,
						Database.userMap.get(msterOrder).getIqImg(), true);// 地主
			}
			int po3 = getNextOrder(msterOrder);
			if (po3 == mySelfOrder) {
				ActivityUtils.setHead(ctx, play1Icon,
						Database.userMap.get(mySelfOrder).getGender(), false,
						Database.userMap.get(mySelfOrder).getIqImg(), true);// 农民
			} else {
				ImageView icon3 = (ImageView) findViewById(1300 + po3);
				if (null != icon3) {
					ActivityUtils.setHead(ctx, icon3, Database.userMap.get(po3)
							.getGender(), false, Database.userMap.get(po3)
							.getIqImg(), true);// 农民
				}
			}
			...
			if (po4 == mySelfOrder) {
				...
				if (null != icon4) {
					ActivityUtils.setHead(ctx, icon4, Database.userMap.get(po4)
							.getGender(), false, Database.userMap.get(po4)
							.getIqImg(), true);// 农民
				}
			}
	...
	public void genxinMycard(String id, String lastCards, int mastOder) {
		...
		if (id.equals(mySelfId)) {
			...
			for (int i = 0; i < 3; i++) {
				...
				ca.getPokeImage()
						.setImageDrawable(
								ImageUtil.getResDrawable(poker[Integer
										.parseInt(dizhuCard[i])]
										.getBitpamResID(), true));
				RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
						mst.adjustXIgnoreDensity(50),
						mst.adjustYIgnoreDensity(68));
		...
		if (str.equals("")) {
			...
			for (int i = 0; i < otherPlayCards.length; i++) {
				...
				c.getPokeImage().setImageDrawable(
						ImageUtil.getResDrawable(poker[Integer
								.parseInt(otherPlayCards[i])].getBitpamResID(),
								true));
			...
			playDiZhuCardAudio(
					type,
					value,
					TextUtils.isEmpty(Database.userMap.get(playOrder)
							.getGender()) ? "0" : Database.userMap.get(
							playOrder).getGender()); // 出牌的声音
			// 如果是炸弹显示特效
	...
	public static int[][] sound_single = {
			{ R.raw.m_1_3, R.raw.m_1_4, R.raw.m_1_5, R.raw.m_1_6, R.raw.m_1_7,
					R.raw.m_1_8, R.raw.m_1_9, R.raw.m_1_10, R.raw.m_1_11,
					R.raw.m_1_12, R.raw.m_1_13, R.raw.m_1_14, R.raw.m_1_15,
					R.raw.m_1_16, R.raw.m_1_17 },
			{ R.raw.w_1_3, R.raw.w_1_4, R.raw.w_1_5, R.raw.w_1_6, R.raw.w_1_7,
					R.raw.w_1_8, R.raw.w_1_9, R.raw.w_1_10, R.raw.w_1_11,
					R.raw.w_1_12, R.raw.w_1_13, R.raw.w_1_14, R.raw.w_1_15,
					R.raw.w_1_16, R.raw.w_1_17 }, };
	public static int[][] sound_pair = {
			{ R.raw.m_2_3, R.raw.m_2_4, R.raw.m_2_5, R.raw.m_2_6, R.raw.m_2_7,
					R.raw.m_2_8, R.raw.m_2_9, R.raw.m_2_10, R.raw.m_2_11,
					R.raw.m_2_12, R.raw.m_2_13, R.raw.m_2_14, R.raw.m_2_15 },
			{ R.raw.w_2_3, R.raw.w_2_4, R.raw.w_2_5, R.raw.w_2_6, R.raw.w_2_7,
					R.raw.w_2_8, R.raw.w_2_9, R.raw.w_2_10, R.raw.w_2_11,
					R.raw.w_2_12, R.raw.w_2_13, R.raw.w_2_14, R.raw.w_2_15 }, };
	public static int[][] sound_three = {
			{ R.raw.m_tuple3, R.raw.m_tuple4, R.raw.m_tuple5, R.raw.m_tuple6,
					R.raw.m_tuple7, R.raw.m_tuple8, R.raw.m_tuple9,
					R.raw.m_tuple10, R.raw.m_tuple11, R.raw.m_tuple12,
					R.raw.m_tuple13, R.raw.m_tuple14, R.raw.m_tuple15, },
			{ R.raw.w_tuple3, R.raw.w_tuple4, R.raw.w_tuple5, R.raw.w_tuple6,
					R.raw.w_tuple7, R.raw.w_tuple8, R.raw.w_tuple9,
					R.raw.w_tuple10, R.raw.w_tuple11, R.raw.w_tuple12,
					R.raw.w_tuple13, R.raw.w_tuple14, R.raw.w_tuple15, } };
			...
			if (value > 0) {
				...
				if (value >= 0 && value < sound_single[0].length) {
					AudioPlayUtils.getInstance().playSound(
							sound_single["1".equals(gender) ? 1 : 0][value]); // 出牌
				}
			}
			break;
		case DoudizhuRule.Yidui:// 如果是两张牌
			...
			if (value > 0) {
				...
				if (value >= 0 && value < sound_pair[0].length) {
					AudioPlayUtils.getInstance().playSound(
							sound_pair["1".equals(gender) ? 1 : 0][value]); // 出牌
				}
			}
			break;
		case DoudizhuRule.wangzha:// 如果是两张牌 王炸
			...
			if (value > 0) {
				...
				if (value >= 0 && value < sound_three[0].length) {
					AudioPlayUtils.getInstance().playSound(
							sound_three["1".equals(gender) ? 1 : 0][value]); // 出牌
				}
			}
			/*
			 * if ("1".equals(gender)) {
			 * AudioPlayUtils.getInstance().playMultiMusic2(R.raw.nv_wangzha,
			 * R.raw.boombeffect); } else {
		...
		if (0 != masterOrder) {
			...
			if (Math.abs(System.currentTimeMillis() - Constant.CLICK_TIME) >= Constant.SPACING_TIME) {// 防止重复刷新
				...
				try {
					...
					PhotoDialog photoDialog = new PhotoDialog(view, isMaster,
							DoudizhuMainGameActivity.this, showAccount, gu);
			...
			if (gu.getLoginToken().endsWith(mess.getFromUserId())
				...
				if (i == play2O) {
					...
					if (mess.getType() == Constant.MESSAGE_TYPE_THREE) {
						...
						// messageFrame(girlRightFrame, null, 0, mess,
						// textView);
					} else {
	...
	public void messageFrame(final LinearLayout layout, String talk,
		...
		if (null == talk) {
			...
			// String name = mess.getValue().substring(0,
			// mess.getValue().lastIndexOf(".")) +
			// "_left";
			// girlLeftTv.setText(ImageUtil.getGirlSayText(name));
			// layout.setBackgroundDrawable(ImageUtil.getResDrawableByName(name,
			// true, true));
			// }
			...
			// }
			// if (-1 == indext) {
			// Drawable drawable = ImageUtil.getResDrawableByName(name, true,
			// true);
			// // Drawable转Bitmap
			...
			// if (bitmap == null) {
			// bitmap = ImageUtil.resizeBitmap(bd.getBitmap(), width, height -
			// 1);
			// ImageUtil.addBitMap2Cache(bitMapKey, bitmap);
			// }
			// // Bitmap bitmap =
			...
			// layout.setBackgroundDrawable(ImageUtil.getResDrawableByName(talk,
			// true, true));
			// int height = Database.SCREEN_HEIGHT;
			// int width = Database.SCREEN_WIDTH;
			// Log.i("Background", "left-- height:" + height + "  width: " +
			// width + " name: " +
			// talk);
			// }
		}
	...
	private class GameCallBack implements ICallback {
		...
		public synchronized void messageHandler(CmdDetail cmdDtl) {
			...
			if (CmdUtils.CMD_GEN_LANDOWNERS.equals(cmd)) {// 产生地主(带踢拉功能)
				hideSlowTip();
				Message message = new Message();
				GenLandowners gld = JsonHelper.fromJson(detail,
						GenLandowners.class); // jsonData是一个Json对象
				message.what = 9;
				Bundle bundle = new Bundle();
			...
			if (CmdUtils.CMD_SENDLASTCARDS.equals(cmd)) {// 发剩余的牌给地主(带踢拉功能)
				hideSlowTip();
				Message message = new Message();
				LastCards lastCard = JsonHelper.fromJson(detail,
						LastCards.class); // jsonData是一个Json对象
				message.what = 10;
				Bundle bundle = new Bundle();
			...
			if (CmdUtils.CMD_END.equals(cmd)) {// 如果是打完牌的話)
				...
				Message message = new Message();
				LinkedList<Play> playResult = JsonHelper.fromJson(detail,
						new TypeToken<LinkedList<Play>>() {
						});
				message.what = 4;
				Bundle bundle = new Bundle();
			...
			if (CmdUtils.CMD_USER.equals(cmd)) { // 用户信息
				Database.userMap = JsonHelper.fromJson(detail,
						new TypeToken<Map<Integer, GameUser>>() {
						}); // 用户的基本信息
				if (Database.userMap != null) {
					GameUser users = null;
					GameUser cacheUser = (GameUser) GameCache
					...
					for (GameUser gu : Database.userMap.values()) {
						...
						if (gu.getAccount().equals(cacheUser.getAccount())) {
							...
							ActivityUtils.addSharedValue(
									RecordPorkerView.MSG_TIPS, jiPaiQiTipsMsg);
							Log.i("gameUserInfo", ""
									+ users.getIqImg().toString());
							break;
						} else {
							jiPaiQiTipsMsg = ActivityUtils
									.getSharedValue(RecordPorkerView.MSG_TIPS);
						}
					}
					if (null != users) {
			...
			if (CmdUtils.CMD_ROUTING.equals(cmd)) { // 收到服务器返回的快速游戏房间信息
				String mess = detail;
				if (mess != null && mess != "") {
					Database.JOIN_ROOM = JsonHelper.fromJson(mess,
							new TypeToken<Room>() {
							});
					Database.JOIN_ROOM_RATIO = Database.JOIN_ROOM.getRatio();
					Database.JOIN_ROOM_BASEPOINT = Database.JOIN_ROOM
							.getBasePoint();
			...
			if (CmdUtils.CMD_ERR_RJOIN.equals(cmd)) { // 加入游戏校验失败
				if (HttpRequest.NO_LOGIN.equals(detail)
						|| HttpRequest.TOKEN_ILLEGAL.equals(detail)) { // 未登录,用户登录token非法
			...
			if (CmdUtils.CMD_HDETAIL.equals(cmd)) { // 加入游戏校验失败，金豆不足
				...
				try {
					...
					boolean isRecharge = PayTipUtils.showTip(b,
			...
			if (CmdUtils.CMD_RANK.equals(cmd)) { // 排名（普通赛制）
				...
				final List<GameScoreTradeRank> gstList = JsonHelper.fromJson(
						detail, new TypeToken<ArrayList<GameScoreTradeRank>>() {
						});
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						mrDialog = new MatchRankDialog(ctx, R.style.dialog,
								gstList);
						mrDialog.setContentView(R.layout.match_rank_dialog);
						android.view.WindowManager.LayoutParams lay = mrDialog
								.getWindow().getAttributes();
						setParams(lay);
						mrDialog.show();
						...
						for (int i = 0, count = gstLists.size(); i < count; i++) {// 获取自己的排名和积分，用于刷新打牌界面的积分和排名
							...
							if (cacheUser.getAccount().equals(
									gctRank.getAccount())) {
								cacheUser.setRank(Integer.parseInt(gctRank
										.getRank().trim()));
								cacheUser.setCred(Long.parseLong(gctRank
										.getScore().trim()));
								GameCache.putObj(CacheKey.GAME_USER, cacheUser);
							}
						}
			...
			if (CmdUtils.CMD_BREAK.equals(cmd)) {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						if (breakOrder == getNextOrder(mySelfOrder)) { // 我的下家断线
							play2Icon.setImageDrawable(ImageUtil
									.getResDrawable(R.drawable.robot, true));
							play2IsTuoGuan = true;
						} else if (breakOrder == getPerOrder(mySelfOrder)) {// 我的上家断线
							play3Icon.setImageDrawable(ImageUtil
									.getResDrawable(R.drawable.robot, true));
							play3IsTuoGuan = true;
						}
					}
			...
			if (CmdUtils.CMD_PPC.equals(cmd)) {
				...
				try {
					...
					status = jsonObject
							.getString(PrerechargeManager.PRERECHARGE_ORDER_PARAMS_STATUS);
					descrption = jsonObject
							.getString(PrerechargeManager.PRERECHARGE_ORDER_PARAMS_DETAIL);
					preOderNo = jsonObject
							.getString(PrerechargeManager.PRERECHARGE_ORDER_PARAMS_PREORDERNO);
				} catch (JSONException e) {
					e.printStackTrace();
				}
				if (null != status && status.equalsIgnoreCase(Constant.SUCCESS)) {
					if (null == descrption)
						descrption = Database.currentActivity
								.getString(R.string.text_prerecharge_order_create_success);
					...
					DialogUtils
							.toastTip(Database.currentActivity
									.getString(R.string.text_prerecharge_order_create_failed));
				}
				return;
			}
		}
	...
	private void setUserRankInfo(GameUser gameUser) {
		if (gameUser.isPlay()) {// 是否为比赛场
			...
			GameCache.putObj(CacheKey.GAME_USER, cacheUser);
			Log.d("detail",
					" 第" + gameUser.getRank() + "名，积分：" + gameUser.getCred());
		...
		try {
			...
			if (relink.getTarget() == ReLink.TARGET_OTHER) {
				...
				if (relinkOrder == getNextOrder(mySelfOrder)) { // 我的下家连线
					...
					ActivityUtils.setHead(ctx, play2Icon,
							Database.userMap.get(p2o).getGender(),
							(p2o == masterOrder), Database.userMap.get(p2o)
									.getIqImg(), false);// 设置头像
				} else if (relinkOrder == getPerOrder(mySelfOrder)) {// 我的上家连线
					int p3o = getPerOrder(mySelfOrder);
					GameUser mGameUser3 = Database.userMap.get(p3o);
					ActivityUtils.setHead(ctx, play3Icon,
							mGameUser3.getGender(), (p3o == masterOrder),
							mGameUser3.getIqImg(), false);// 设置头像
				}
				return;
			}
			showPopWindow(false);
			...
			if ((6 == Database.JOIN_ROOM.getHomeType() && quang < allQuan)
					|| (6 != Database.JOIN_ROOM.getHomeType() && quang < 5)) {
				handler.sendEmptyMessage(302);
			}
			// 用户
			...
			playTextView1.setText((nickName != null) ? nickName : tempUser
					.getGameUser().getAccount()); // 自己
			ActivityUtils.setHead(ctx, play1Icon,
					Database.userMap.get(mySelfOrder).getGender().trim(),
					(mySelfOrder == relink.getMasterOrder()), Database.userMap
							.get(mySelfOrder).getIqImg(), false);// 设置头像
			Log.i("gameUserInfo", "断网重连："
					+ Database.userMap.get(mySelfOrder).getIqImg().toString());
			play1Order.setText(String.valueOf(mySelfOrder));
			...
			try {
				for (Entry<String, String> entry : dpMap.entrySet()) {
					...
					if (!TextUtils.isEmpty(path)) {
						dpRl1.removeAllViews();
						ImageUtil.setImg(HttpURL.URL_PIC_ALL + path, null,
								new ImageCallback() {
									@Override
			...
			Database.userMap.put(tempUser.getOrder(), tempUser.getGameUser());
			nickName = tempUser.getGameUser().getNickname();
			playTextView2.setText(!TextUtils.isEmpty(nickName) ? nickName
					: tempUser.getGameUser().getAccount()); // 下家
			ActivityUtils.setHead(ctx, play2Icon, Database.userMap.get(p2o)
					.getGender(), (p2o == relink.getMasterOrder()),
					Database.userMap.get(p2o).getIqImg(), false);// 设置头像
			play2Order.setText(String.valueOf(p2o));
			...
			Database.userMap.put(tempUser.getOrder(), tempUser.getGameUser());
			nickName = tempUser.getGameUser().getNickname();
			playTextView3.setText((nickName != null) ? nickName : tempUser
					.getGameUser().getAccount()); // 上家
			ActivityUtils.setHead(ctx, play3Icon, Database.userMap.get(p3o)
					.getGender(), (p3o == relink.getMasterOrder()),
					Database.userMap.get(p3o).getIqImg(), false);// 设置头像
			play3Order.setText(String.valueOf(p3o));
			...
			if (relStatus == Constant.STATUS_PLAYING) { // 正在打牌时重连
				...
				try {
					...
					String[] dzCard = masterCards.substring(1,
							masterCards.length() - 1).split(",");
					// 添加地主的牌在桌面上
					dizhuPaiLayout.removeAllViews();
					for (int i = 0; i < dzCard.length; i++) {
						...
						ca.getPokeImage().setImageDrawable(
								ImageUtil.getResDrawable(poker[Integer
										.parseInt(dzCard[i])].getBitpamResID(),
										true));
						RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
								mst.adjustXIgnoreDensity(50),
								mst.adjustYIgnoreDensity(68));
				...
				if (isMyDo) { // 轮到自己打牌
					...
					ImageView img = (ImageView) findViewById(relink
							.getNextPlayOrder() + 1400);
					img.setVisibility(View.GONE);
				}
				return;
			}
		...
		if (playCardSize > 0) { // 别人有出牌
			for (int i = playCardSize - 1; i >= 0; i--) {
				...
				if (tmpOrder == mySelfOrder) { // 自己出的牌
					List<Integer> pcardList = JsonHelper.fromJson(
							playCardList.get(i),
						...
						for (int j = 0; j < passPokerList.size(); j++) {
							...
							image.getPokeImage().setImageDrawable(
									ImageUtil.getResDrawable(
											passPokerList.get(j)
													.getBitpamResID(), true));
							params3.leftMargin = mst
									.adjustXIgnoreDensity(20 * j);
							play1PassLayout.addView(image, params3);// 显示出来
							// 清除图片
							ImageUtil.releaseDrawable(passPokerList.get(j)
									.getPokeImage().getDrawable());
						}
					}
					hiddenPlayBtn();
				} else {
					...
					p.setCount(relink.getUserInfo().get(tmpOrder)
							.getCardCount()); // 剩余牌
	...
	private void startJoinGame() {
		...
		if (cacheuser == null && type == Constant.FASTJOIN_TYPE) {
			...
			if (type == Constant.FASTJOIN_TYPE) {
				...
				// 发送加入游戏命令
				CmdUtils.sendJoinRoomCmd(cacheuser.getLoginToken(),
						String.valueOf(Database.JOIN_ROOM_CODE));
			}
		}
	}
			...
			if (PreferenceHelper.getMyPreference().getSetting()
				...
				if (tilaLayout.getVisibility() == View.VISIBLE
					...
					if (e.getRawX() < left || e.getRawX() > right
						...
						jiaBei4 = false;
						if (jia == 2) {
							tiLaBtn2.setBackgroundDrawable(ImageUtil
									.getResDrawable(R.drawable.jiabei_2_touch,
											true));
							tiLaBtn4.setBackgroundResource(R.drawable.jiabei_btn_bg_4);
							jiaBei2 = true;
						} else if (jia == 4) {
							tiLaBtn2.setBackgroundResource(R.drawable.jiabei_btn_bg_2);
							tiLaBtn4.setBackgroundDrawable(ImageUtil
									.getResDrawable(R.drawable.jiabei_4_touch,
											true));
							jiaBei4 = true;
							jia = 0;
						}
		...
		ScheduledTask.addDelayTask(new AutoTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						canFlipper = true;
						int curId = DoudizhuMainGameActivity.this.viewFlipper
								.getDisplayedChild();
						int point = curPage - 1;
						...
						if (null != draw) {
							DoudizhuMainGameActivity.this.viewFlipper
									.getChildAt(curpoint)
			...
			public void onAnimationEnd(Animation animation) {
				...
				case IS_ZHADAN_ANIM:
					...
					AnimUtils.playAnim(zhadanImageView,
							ImageUtil.getResAnimaSoft("bomb"), 1500);
					break;
				default:
					break;
				}
