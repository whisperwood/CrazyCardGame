...
public class PayRecordActivity extends BaseActivity {
	...
	private OnItemClickListener onItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
			...
			if (null == payRecordOrderInfo)
				return;
			if (payRecordOrderInfo.get(PayRecordUtil.ORDER_TYPE).equalsIgnoreCase(
					PayRecordUtil.OrderType.Order_prepay.toString())
					&& payRecordOrderInfo.get(PayRecordUtil.PAY_STATUS).equalsIgnoreCase(
							PayRecordUtil.RecordStatus.Record_freeze.toString())) {
				PrerechargeManager.createPrerechargeDialog(PrerechargeDialogType.Dialog_record, PayRecordActivity.this,
						recordAdapter.getPayRecordOrder(position), mHandler, 0).show();
			}
		}
	};
			...
			try {
				...
				if (HttpRequest.FAIL_STATE.equals(resultJson)) {
					...
					if (null == resultJson || resultJson.trim().equals("")) {
						...
						final List<PayRecordOrder> payRecordOrders = JsonHelper.fromJson(resultJson,
	...
	private class RecordAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (payRecordOrdersInfo.get(position).get(PayRecordUtil.PAY_STATUS)
				...
				mViewHolder.status.setBackgroundDrawable(ImageUtil.getDrawableResId(R.drawable.charge_status_complete,
						false, false));
			} else {
				(convertView.findViewById(R.id.reocord_item)).setBackgroundResource(R.drawable.pre_recharge_item_bg);
				mViewHolder.status.setBackgroundDrawable(ImageUtil.getDrawableResId(R.drawable.charge_status_freeze,
						false, false));
			}
