...
public class TaskMenuActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (null != TaskMenuMap && TaskMenuMap.containsKey(Constant.SMS_INITE_MSG)) {
			codeTipMsg.setText(TextUtils.isEmpty(TaskMenuMap.get(Constant.SMS_INITE_MSG)) ? "" : TaskMenuMap
		...
		if (null != TaskMenuMap && TaskMenuMap.containsKey(Constant.SMS_INITE_CODE)) {
			telphoneTextView.setText(TextUtils.isEmpty(TaskMenuMap.get(Constant.SMS_INITE_CODE)) ? "" : TaskMenuMap
		...
		if ("close".equals(emsSwitch)) {
			...
			if (null != TaskMenuMap && TaskMenuMap.containsKey(Constant.SMS_INITE_INPUT)) {
				telphoneTextView.setText(TextUtils.isEmpty(TaskMenuMap.get(Constant.SMS_INITE_INPUT)) ? ""
						: TaskMenuMap.get(Constant.SMS_INITE_INPUT));
			} else {
				telphoneTextView.setText("");
			}
		}
		...
		if (null != TaskMenuMap && TaskMenuMap.containsKey(Constant.FILL_IN_INFO)) {
			userinfoLable.setText(TextUtils.isEmpty(TaskMenuMap.get(Constant.FILL_IN_INFO)) ? "" : TaskMenuMap
		...
		if (null != TaskMenuMap && TaskMenuMap.containsKey(Constant.INVITE_FRIEND)) {
			frientsLabel.setText(TextUtils.isEmpty(TaskMenuMap.get(Constant.INVITE_FRIEND)) ? "" : TaskMenuMap
		...
		if (null != TaskMenuMap && TaskMenuMap.containsKey(Constant.APP_DOWNLOAD)) {
			textTip.setText(TextUtils.isEmpty(TaskMenuMap.get(Constant.APP_DOWNLOAD)) ? "" : TaskMenuMap
		...
		private void showDownLoadView(String isOpen) {
			...
			if ("1".equals(isOpen)) {
				...
				if (null != TaskMenuMap && TaskMenuMap.containsKey(Constant.FUN_NO_OPEN)) {
					textTip.setText(TextUtils.isEmpty(TaskMenuMap.get(Constant.FUN_NO_OPEN)) ? "" : TaskMenuMap
			...
			gridView.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View arg1, final int arg2, long arg3) {
					if (arg1.findViewById(R.id.apk_down).getVisibility() == View.VISIBLE) {
						...
						try {
							packageInfo = TaskMenuActivity.this.getPackageManager().getPackageInfo(
	...
	/**
	 * 用于获取intent和pageView， 类似于单例模式，使得对象不用重复创建，同时，保留上一个对象的状态 当重新访问时，仍保留原来数据状态，如文本框里面的值。
	...
	OnClickListener clickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if (v.getId() == R.id.invitation_msg_btn || v.getId() == R.id.bott_telphone_btn
					|| v.getId() == R.id.bott_userinfo_btn || v.getId() == R.id.invitation_friend_btn
					...
					if (!isMobile) {
						...
						if (ActivityUtils.simExist()) { // 判断SIM卡是否能使用
							...
							if ("close".equals(emsSwitch)) {
								...
								PendingIntent sentIntent = PendingIntent.getBroadcast(TaskMenuActivity.this, 0, intent,
										0);
								smsManager.sendTextMessage(mobile, null, "掌中游斗地主金豆大赠送，完善手机号获取金豆验证码:" + validatorCode,
										sentIntent, null);
							}
						}
					}
			...
			holder.downBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (((Button) v).getText().toString().equals("打开")) {
						Intent startIntent = context.getPackageManager().getLaunchIntentForPackage(
								soft.getPackageName());
						...
						if (!ActivityUtils.isOpenWifi()) {
							GameDialog gameDialog = new GameDialog(Database.currentActivity) {
								...
								public void okClick() {
									...
									UpdateUtils.downApk(TaskMenuActivity.this, soft.getName(), soft.getDownUrl(),
							...
							Toast.makeText(TaskMenuActivity.this, soft.getName() + "应用正在下载中...", Toast.LENGTH_SHORT)
									.show();
