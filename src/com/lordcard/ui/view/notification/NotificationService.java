						...
						if (Database.PUSH_DATA != null) {
							...
							if (!TextUtils.isEmpty(notices)) {
								...
								if ("1".equals(pushData.getType())) {// 1:推送通知
									Notifications.activityNotification(getApplicationContext(),
											StartActivity.class.getName(), pushData.getTitle(), pushData.getContent());
								} else if ("6".equals(pushData.getType())) {// 6:命令推送
									if (!TextUtils.isEmpty(pushData.getContent())) {
										GameCommandCheck gcc = JsonHelper.fromJson(pushData.getContent(),
		...
		timer.schedule(new TimerTask() {
			...
			private void checkGameTime() {
				if (null != roomStartTimeMap) {
					...
					Log.i("", "roomStartTimeMap:" + it.hasNext() + "                        roomEndTimeMap:"
					...
					try {
						if (null != gcc.getCommandContentMap()) {
							...
							if (commContent.containsKey("pingUrl") && commContent.containsKey("pingCount")) {
								ReturnPing rPing = ActivityUtils.getPing(commContent.get("pingUrl"),
										Integer.parseInt(commContent.get("pingCount")));// ping信息
								if (null == rPing) {
