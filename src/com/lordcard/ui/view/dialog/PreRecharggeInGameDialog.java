...
public class PreRecharggeInGameDialog extends BaseDialog {
	...
	public void initContentView(Context context) {
		...
		mainLayout = findViewById(R.id.pre_recharge_root_ingame);
		...
		price = PrerechargeManager.calculatePrerechargePrice(
				cacheUser.getBean(), multiple * Database.JOIN_ROOM_BASEPOINT);
		if (price[0] <= 0) {
			price[0] = 2;
		}
		if (price[1] <= 0) {
		...
		text_charge_One.setText(getStringWithParams(R.string.text_charge_w,
				price[0]));
		int length = -1;
		int index = -1;
		// String paramString = PatternUtils.formatIqBeans(((totalWager >
		// (cacheUser.getBean() +
		// price[0]*100000))?(cacheUser.getBean() + price[0]*100000) :
		// totalWager));
		String paramString = PatternUtils.formatIqBeans(cacheUser.getBean()
				+ price[0] * 10000);
		String tempText = getStringWithParams(R.string.text_can_win_beans,
				paramString);
		...
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		canWinBeans_One.setText(spannableStringBuilder);
		text_charge_Two = (TextView) findViewById(R.id.pre_recharge_text_two);
		text_charge_Two.setText(getStringWithParams(R.string.text_charge_w,
				price[1]));
		canWinBeans_Two = (TextView) findViewById(R.id.textview_can_win_beans_two);
		// paramString = PatternUtils.formatIqBeans(((totalWager >
		// (cacheUser.getBean() +
		// price[1]*100000))?(cacheUser.getBean() + price[1]*100000) :
		// totalWager));
		paramString = PatternUtils.formatIqBeans(cacheUser.getBean() + price[1]
				* 10000);
		...
		tempText = getStringWithParams(R.string.text_pre_recharge_notice_one,
				tempStringOne, tempStringTwo);
		spannableStringBuilder = new SpannableStringBuilder(tempText);
		length = tempStringOne.length();
		index = tempText.indexOf(tempStringOne);
		spannableStringBuilder.length();
		spannableStringBuilder.setSpan(new AbsoluteSizeSpan(20, true), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(
				new ForegroundColorSpan(Color.parseColor("#b71600")), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		...
		spannableStringBuilder.setSpan(new AbsoluteSizeSpan(20, true), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(
				new ForegroundColorSpan(Color.parseColor("#8c5209")), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		...
		tempText = getStringWithParams(getStringWithParams(
				R.string.text_pre_recharge_bureau_wager,
				PatternUtils.formatIqBeans(totalWager)));
		spannableStringBuilder = new SpannableStringBuilder(tempText);
		...
		tempText = getStringWithParams(R.string.text_pre_recharge_notice_two,
				paramString);
		spannableStringBuilder = new SpannableStringBuilder(tempText);
		index = tempText.indexOf(paramString);
		length = paramString.length();
		...
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new AbsoluteSizeSpan(19, true), index,
				index + length, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
