...
public class PreRecharggeInGameDialog extends BaseDialog {
	...
	public void initContentView(Context context) {
		...
		price = PrerechargeManager.calculatePrerechargePrice(cacheUser.getBean(), multiple
		...
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index, index + length,
		...
		spannableStringBuilder.setSpan(new AbsoluteSizeSpan(20, true), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new ForegroundColorSpan(Color.parseColor("#b71600")), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		...
		spannableStringBuilder.setSpan(new AbsoluteSizeSpan(20, true), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new ForegroundColorSpan(Color.parseColor("#8c5209")), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		...
		tempText = getStringWithParams(getStringWithParams(R.string.text_pre_recharge_bureau_wager,
		...
		spannableStringBuilder.setSpan(new StyleSpan(Typeface.BOLD), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		spannableStringBuilder.setSpan(new AbsoluteSizeSpan(19, true), index, index + length,
				Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		rechargeNotice.setText(spannableStringBuilder);
		/** Initialize buttons for dialog **/
		initButtons(null, null, (Button) mainLayout.findViewById(R.id.pre_recharge_ingame_btn_cancel));
	}
