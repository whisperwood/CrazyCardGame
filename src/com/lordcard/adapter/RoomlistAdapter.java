...
public class RoomlistAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		// JsonHelper.fromJson(room.getStartRace(), new TypeToken<Map<String,
		// String>>() {});
	...
	public class ViewHolder {
		...
		private boolean isAddPerson() {
			...
			/*
			 * if(hour>=6&&hour<24) { addRate =90; } else if(hour>=0&&hour<3) {
			 * addRate =20; } else if(hour>=3&&hour<6) { addRate =10; }
			 */
			// 八点到10点
			if (hour >= 20 && hour < 22) {
				addRate = 80;
		...
		private void setCount() {
			...
			/*
			 * if(isAddPerson) { int baseCount = 2+r.nextInt(10); count =
			 * r.nextInt(baseCount); }else { count = -count; }
			 */
			if (!loading) {
