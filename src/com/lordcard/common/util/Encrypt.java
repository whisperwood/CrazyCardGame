...
public class Encrypt {
	...
	public static String encrypt(String batchId, String mac, String sysbId) {
		...
		for (int i = 0; i < macArray.length; i++) {
			...
			String tempBatchId = new Integer((Integer.parseInt(batchId
					.substring(i, i + 1)) + 5) % 10).toString();
			sb.append(tempMac.substring(0, 1) + tempBatchId
					+ tempMac.substring(1));
		}
		/** 将sysbId拆分成2部分，第一部分插入到sb的第10位之后，第二部分插入到sb第13位后面 */
		String newTargetStr = null;
		newTargetStr = sb.subSequence(0, 10).toString()
				+ sysbId.substring(0, 1).toString()
				+ sb.substring(10, 13).toString() + sysbId.substring(1)
				+ sb.substring(13).toString();
		/** 将newTargetStr中的0换成B，2换成5，A换成7，D换成C */
		newTargetStr = newTargetStr.replaceAll("0", "B").replaceAll("2", "5")
				.replaceAll("A", "7").replaceAll("D", "C");
