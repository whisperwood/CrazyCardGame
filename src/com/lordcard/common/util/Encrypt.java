...
public class Encrypt {
	/**
	 * 功能说明：将批次号、MAC地址、应用ID一起加密 参数说明：@param batchId 批次号 参数说明：@param mac MAC地址 参数说明：@param sysbId
	 * 应用ID 参数说明：@return 加密后的密码 返回值说明:String 创建者:xupeng 创建日期:2012-8-2
	 */
	...
	public static String encrypt(String batchId, String mac, String sysbId) {
		...
		newTargetStr = sb.subSequence(0, 10).toString() + sysbId.substring(0, 1).toString()
				+ sb.substring(10, 13).toString() + sysbId.substring(1) + sb.substring(13).toString();
