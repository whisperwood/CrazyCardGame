...
public class AssistantBean {
	...
	public String createTable() {
		...
		return "create table if not exists "
				+ TABLE_NAME
				+ " (id integer primary key,icon text,xicon text,display integer,content text,btnAc text,level integer,validTime text,condition text,title text,joinCode text,pushTime text,type integer,aorder integer)";
		// return "create table if not exists "+
		// TABLE_NAME+" (id integer primary key,icon text)";
		// return "create table if not exists "+
		// TABLE_NAME+" (id integer primary key,icon text,validTime text)";
	}
	...
	private List<HashMap<String, Object>> cursor2List(Cursor cursor, DBHelper dbHelper) {
		...
		// String icon = cursor.getString(cursor.getColumnIndex(AS_ICON));
		// // String display =
		// cursor.getString(cursor.getColumnIndex(AS_DISPLAY));
		// // String content=
		// cursor.getString(cursor.getColumnIndex(AS_CONTENT));
		// // String btnAc= cursor.getString(cursor.getColumnIndex(AS_BTNAC));
		// // String validTime=
		// cursor.getString(cursor.getColumnIndex(AS_TIME));
		// map.put(AS_ID, id);
		// map.put(AS_ICON, icon);
		// // map.put(AS_DISPLAY, display);
		// // map.put(AS_CONTENT, content);
